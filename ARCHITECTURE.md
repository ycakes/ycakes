...
- **"View Details" (`OrderDetailModal`)**: originally skipped as out of scope, then built after user feedback that the compact card alone wasn't enough. A Base UI `Dialog` that lazily fetches full order details on open (line items with prices, fulfillment method/area/date, address, notes, subtotal/discount/total) scoped to the caller via the same `orders_select_own` RLS — no separate route/page needed.
