...
import { createManualOrder, emptyFieldsValue, type ManualOrderItem } from "@/lib/admin/manualOrder";
...
export function NewOrderContent({
  ...
  const deliveryFee = fulfillmentMethod === "delivery" ? (deliveryArea?.price ?? 0) : 0;
  ...
  async function handleCreate() {
    ...
    if (!guestName || !phone.trim() || validItems.length === 0 || !fulfillmentMethod || !fulfillmentDate) {
      setError(t("requiredFieldsError"));
      return;
    }
    if (fulfillmentMethod === "delivery" && (!deliveryAreaId || !address.trim())) {
      setError(t("requiredFieldsError"));
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-heading text-2xl font-bold text-brand-primary">{t("newOrderTitle")}</h1>
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
        <div className="flex w-full flex-col gap-6 lg:max-w-[728px]">
          <div className="flex flex-col gap-4 rounded-3xl border border-border-default bg-bg-surface p-6">
            <p className="text-xl font-semibold text-text-primary">{t("orderSource")}</p>
            <p className="text-[13px] text-text-secondary">{t("orderSourceHint")}</p>
            <div className="flex flex-wrap gap-3">
              {SOURCES.map((s) => (
                <SelectChip key={s} label={t(`sourceValue.${s}`)} selected={source === s} onSelect={() => setSource(s)} />
              ))}
            </div>
          </div>

          <div className="flex flex-col gap-4 rounded-3xl border border-border-default bg-bg-surface p-6">
            <p className="text-xl font-semibold text-text-primary">{t("customerDetails")}</p>
            <div className="flex gap-3">
              <InputField label={tCheckout("firstName")} value={firstName} onChange={setFirstName} />
              <InputField label={tCheckout("lastName")} value={lastName} onChange={setLastName} />
            </div>
            <div className="flex flex-col gap-2">
              <InputField label={t("phoneNumber")} type="tel" value={phone} onChange={setPhone} />
            ...
            <InputField label={t("addressDeliveryHint")} value={address} onChange={setAddress} />
            <InputField label={tCheckout("emailOptional")} type="email" value={email} onChange={setEmail} />
          </div>

          <div className="flex flex-col gap-4 rounded-3xl border border-border-default bg-bg-surface p-6">
            <p className="text-xl font-semibold text-text-primary">{t("orderItems")}</p>
            {items.map((item) => (
              <NewOrderItemRow
                key={item.key}
                item={item}
          ...
          <div className="flex flex-col gap-4 rounded-3xl border border-border-default bg-bg-surface p-6">
            <p className="text-xl font-semibold text-text-primary">{t("orderNotesTitle")}</p>
            <InputField label="" placeholder={t("orderNotesPlaceholder")} value={notes} onChange={setNotes} multiline />
          </div>
        </div>
        ...
        <div className="flex w-full flex-col gap-6 lg:max-w-[376px]">
          <div className="flex flex-col gap-3.5 rounded-3xl border border-border-default bg-bg-surface p-6">
            <p className="text-xl font-semibold text-text-primary">{t("fulfillment")}</p>
            <div className="flex flex-wrap gap-2.5">
              ...
              />
              <SelectChip label={t("delivery")} selected={fulfillmentMethod === "delivery"} onSelect={() => setFulfillmentMethod("delivery")} />
            </div>
            {fulfillmentMethod === "delivery" && (
              <>
                <p className="text-[13px] font-medium text-text-primary">{t("deliveryArea")}</p>
                <div className="flex flex-wrap gap-2.5">
                  {deliveryAreas.map((area) => (
                    <SelectChip key={area.id} label={area.name[locale]} selected={deliveryAreaId === area.id} onSelect={() => setDeliveryAreaId(area.id)} />
                  ))}
                </div>
              </>
            )}
            <p className="text-[13px] font-medium text-text-primary">{t("deliveryDate")}</p>
            <div className="relative w-full">
              ...
              {dateOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setDateOpen(false)} aria-hidden="true" />
          ...
          <div className="flex flex-col gap-3.5 rounded-3xl border border-border-default bg-bg-surface p-6">
            <p className="text-xl font-semibold text-text-primary">{t("orderSummary")}</p>
            <div className="flex items-center justify-between text-sm">
              <span className="text-text-secondary">{t("itemsCount", { count: items.length })}</span>
              <span className="font-medium text-text-primary">{tCommon("egpPrice", { amount: subtotal })}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-text-secondary">{t("deliveryFee")}</span>
              <span className="font-medium text-text-primary">{tCommon("egpPrice", { amount: deliveryFee })}</span>
            </div>
            ...
            <div className="flex items-center justify-between font-semibold text-text-primary">
              ...
              <span className="text-lg">{tCommon("egpPrice", { amount: total })}</span>
            </div>
            {error && <p className="text-xs text-red-600">{error}</p>}
            <Button type="button" variant="brand-primary" size="xl" disabled={submitting} onClick={handleCreate} className="w-full justify-center">
              {t("createOrder")}
            </Button>
          </div>
        </div>
