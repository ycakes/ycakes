...
5. **Every UI surface must support EN/AR + RTL** — don't hardcode English-only strings or LTR-only layout assumptions, even in early scaffolding.
6. **All money fields are EGP.** No currency conversion, no other currencies.
7. **Don't introduce new dependencies, services, or architectural patterns without flagging it** — if something in `ARCHITECTURE.md` seems wrong or insufficient mid-task, stop and ask rather than silently deciding differently.
8. Keep changes scoped to the current task in `TASKS.md`. Don't opportunistically refactor unrelated code.
