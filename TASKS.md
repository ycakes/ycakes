...
- [x] Cake Detail + customization flow (`/cakes/[id]`, `CakeCustomizer` client component)
  ...
  - [x] **Icing color is multi-select** (owner request) — any number of colors per item via the new `order_item_colors` table (see ARCHITECTURE.md). Selecting more than one shows a hint to describe the arrangement in Additional Notes; there's no structured per-color placement field.
